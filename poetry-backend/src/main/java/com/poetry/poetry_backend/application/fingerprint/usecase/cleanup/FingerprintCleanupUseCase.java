 ...
 * Purpose: Archives inactive fingerprints.
 * Finds fingerprints with no activity past threshold and archives records.
...
public class FingerprintCleanupUseCase {
    ...
    public FingerprintCleanupResult execute(Instant cutoff, int batchSize) {
        ...
        for (Fingerprint fp : stale) {
            Fingerprint archived = FingerprintArchiver.markArchived(fp);
            commandPort.save(archived);
            archivedCount++;
        }

        return FingerprintCleanupResult.success(archivedCount);
    }
}
