...
/**
 * Contract:
 * Inputs: Lists of available token option DTOs (themes, fonts, etc.) plus
 * access to
 * stored selection and active theme use case.
 * Behavior:
 * 1. If a stored system selection exists and its theme still exists -> use it.
 * 2. If stored exists but theme removed -> substitute first available or
 * "default".
 ...
 * Errors: Never throws; swallows domain active theme exception and proceeds
 * with fallback.
 */
public class ResolveCurrentSelectionUseCase {
  ...
  public UiCustomizationSelection execute(
    ...
    // Prefer explicit preloadDefault font if provided; else first; else fallback
    // literal.
