...
public interface FingerprintJpaRepository
        ...
        @Query("SELECT f FROM FingerprintEntity f WHERE f.status = :status "
                        + "AND f.fmd IS NOT NULL AND f.deletedAt IS NULL")
        List<FingerprintEntity> findByStatusAndFmdIsNotNull(
                        @Param("status") FingerprintStatus status);
