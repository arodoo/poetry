...
test.describe('Membership Success Flow', () => {
    ...
    test('should create a membership successfully', async ({ page }) => {
        ...
        await expect(page.getByTestId('eligibility-checking')).not.toBeVisible({ timeout: 5000 })
        await expect(page.getByTestId('selected-user-label')).toContainText(/admin/i)
