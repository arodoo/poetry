...
test.describe('Membership Form Interactions', () => {
    ...
    test('should validate seller code in real-time', async ({ page }) => {
        ...
        await expect(page.getByTestId('subscription-select')).toBeVisible({ timeout: 5000 })

        await page.getByTestId('subscription-select').selectOption({ label: 'Basic Monthly' })
        ...
        await expect(page.getByTestId('eligibility-error')).toBeVisible({ timeout: 10000 })
    })
    ...
    test('should reset form state when a new user is selected', async ({ page }) => {
