...
test('should apply smooth opacity transition during fetching', async ({ page }) => {
    ...
    await expect(wrapper).toHaveAttribute('data-fetching', 'true', { timeout: 5000 })
    ...
    await expect(wrapper).toHaveAttribute('data-fetching', 'false', { timeout: 5000 })
    await expect(async () => {
        const opacity = await wrapper.evaluate(el =>
            parseFloat(window.getComputedStyle(el).opacity)
        )
