...
test('cancel button navigates back to detail', async ({ page }: { page: Page }) => {
  ...
  const cancelButton = page.getByTestId('cancel-delete-seller-code-button')
  await cancelButton.click()
  await page.waitForURL(new RegExp(`/en/seller-codes/${sellerCodeId}$`), { timeout: 10000 })
  await expect(page).toHaveURL(new RegExp(`/en/seller-codes/${sellerCodeId}$`))
})
