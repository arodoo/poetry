...
test.describe('Access Logs Sub-Dashboard E2E', () => {
  ...
  test('1. Renders ActiveHours Details View with By Hour Tab as default and KPIs', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('button', { hasText: 'Busiest Days' })
    ).toBeVisible()
    await expect(
      page.locator('button', { hasText: '7-Day Trend' })
    ).toBeVisible()
  ...
  test('2. Clicking Busiest Days tab renders the ActiveDaysChart', async ({
    page,
  }) => {
    await page.goto('/en/charts/details/activeHours')
    await page.waitForLoadState('networkidle')
  ...
  test('3. Clicking 7-Day Trend tab renders the AccessLogTrendChart', async ({
    page,
  }) => {
    await page.goto('/en/charts/details/activeHours')
    await page.waitForLoadState('networkidle')
  ...
  test('4. Raw Data table renders with real user contextual data (top 50)', async ({
    page,
  }) => {
    await page.goto('/en/charts/details/activeHours')
    await page.waitForLoadState('networkidle')
    ...
    await expect(
      page.locator('h3', { hasText: 'Raw Data (Top 50)' })
    ).toBeVisible()
    ...
    await expect(
      page.locator('td', { hasText: 'alice@example.com' })
    ).toBeVisible()
    await expect(page.locator('td', { hasText: 'Bob Jones' })).toBeVisible()
  })

  test('5. Empty state handles recentAccessLogs gracefully', async ({
    page,
  }) => {
    await page.route('**/api/v1/dashboard/metrics', async (route) => {
      await route.fulfill({
  ...
  test('6. Navigation Back button works to return to overview', async ({
    page,
  }) => {
    await page.goto('/en/charts/details/activeHours')
    await page.waitForLoadState('networkidle')
