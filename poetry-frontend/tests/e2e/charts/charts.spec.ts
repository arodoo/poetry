...
test.describe('Charts Feature E2E', () => {
    ...
    // Drill-down Tests
    test('4. Drill-down: Users By Status chart navigates correctly', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Usuarios por Estado|Users by Status/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/usersByStatus/);
        await expect(page.locator('h1').filter({ hasText: /Usuarios por Estado|Users by Status/i })).toBeVisible();
    });

    test('5. Drill-down page shows Raw Data table for Users By Status', async ({ page }) => {
        await page.route('**/api/v1/dashboard/metrics**', async (route) => {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify(getMockMetrics({ usersByStatus: { Active: 50, Inactive: 10 } })),
            });
        });
        ...
        await expect(page.getByRole('columnheader', { name: /Clave principal|Key/i })).toBeVisible();
        await expect(page.getByRole('columnheader', { name: /Valor en cantidad|Value/i })).toBeVisible();
    });

    test('6. "Volver" button on details page correctly navigates back', async ({ page }) => {
        await page.route('**/api/v1/dashboard/metrics**', async (route) => {
            await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(getMockMetrics()) });
        });
        await page.goto('/es/charts/details/usersByStatus');
    ...
    test('7. Birthdays This Month chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Cumpleaños este Mes|Birthdays This Month/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/birthdaysThisMonth/);
        await expect(page.locator('h1').filter({ hasText: /Cumpleaños este Mes|Birthdays This Month/i })).toBeVisible();
    });
    ...
    test('9. Most Populated Regions chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Regiones Más Pobladas|Most Populated Regions/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/populatedRegions/);
        await expect(page.locator('h1').filter({ hasText: /Regiones Más Pobladas|Most Populated Regions/i })).toBeVisible();
    });

    test('10. Enrollments Over Time chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Registros en el Tiempo|Enrollments Over Time/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/enrollmentsOverTime/);
        await expect(page.locator('h1').filter({ hasText: /Registros en el Tiempo|Enrollments Over Time/i })).toBeVisible();
    });

    test('11. Memberships By Status chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Estado de Membresías|Memberships Status/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/membershipsByStatus/);
        await expect(page.locator('h1').filter({ hasText: /Estado de Membresías|Memberships Status/i })).toBeVisible();
    });

    test('12. Audit Events chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Eventos de Auditoría|Audit Events/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/eventsByType/);
        await expect(page.locator('h1').filter({ hasText: /Eventos de Auditoría|Audit Events/i })).toBeVisible();
    });

    test('13. Subscriptions By Duration chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Duración de Suscripciones|Subscriptions Duration/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/subscriptionsByDuration/);
        await expect(page.locator('h1').filter({ hasText: /Duración de Suscripciones|Subscriptions Duration/i })).toBeVisible();
    });

    test('14. Scheduled Events chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Eventos Programados|Scheduled Events/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/scheduledEventsByStatus/);
        await expect(page.locator('h1').filter({ hasText: /Eventos Programados|Scheduled Events/i })).toBeVisible();
    });

    test('15. Seller Codes chart navigates and displays data', async ({ page }) => {
        const chartCard = page.locator('h3').filter({ hasText: /Códigos de Vendedor|Seller Codes/i }).locator('..');
        await chartCard.locator('button').click();
        await expect(page).toHaveURL(/.*\/charts\/details\/sellerCodesByStatus/);
        await expect(page.locator('h1').filter({ hasText: /Códigos de Vendedor|Seller Codes/i })).toBeVisible();
    });

    // Edge cases and access
    test('16. Invalid chartId shows proper 404 or empty state', async ({ page }) => {
        await page.route('**/api/v1/dashboard/metrics**', async (route) => {
            await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(getEmptyMetrics()) });
        });
        await page.goto('/es/charts/details/invalidChartId123');

        // It should render the not found message from renderChart default case
        await expect(page.locator('div').filter({ hasText: /No encontrado|Not Found|ui\.common\.notFound/i }).first()).toBeVisible();
    ...
    test('18. Charts grid collapses to one column on mobile', async ({ page }) => {
    ...
    test('19. Charts data fetches successfully and populates components', async ({ page }) => {
        // Adding multilingual support for this specific existence check
        await expect(page.locator('h3').filter({ hasText: /Cumpleaños este Mes|Birthdays This Month/i })).toBeVisible();
    });
