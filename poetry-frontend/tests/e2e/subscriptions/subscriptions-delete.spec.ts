...
async function createTestSubscription(page: Page): Promise<{ id: string; name: string }> {
  ...
  await page.getByTestId('subscription-description-input').fill('Test subscription for deletion')
...
test('delete button navigates to delete confirmation page', async ({
  ...
  const { id: subscriptionId, name: subscriptionName } = await createTestSubscription(page)
  ...
  await expect(page).toHaveURL(new RegExp(`/en/subscriptions/${subscriptionId}/delete$`))
  await page.waitForLoadState('networkidle')
  
  // Check delete confirmation page
  await expect(page.getByRole('heading', { name: 'Delete Subscription' })).toBeVisible({
    timeout: 15000,
  })
...
test('confirm deletion removes subscription from list', async ({
  ...
  await expect(page.getByText(subscriptionName)).not.toBeVisible({ timeout: 5000 })
  await expect(page.getByTestId(`view-subscription-${subscriptionId}`)).not.toBeVisible({ timeout: 5000 })
})
