...
test.describe('Edit User — Fingerprint Re-enrollment', () => {
  test('shows enrollment section and replaces fingerprint on success', async ({
    ...
    await page.route(
      ...
      async (r: Route) => {
        const body = JSON.parse((r.request().postData() as string) || '{}') as {
          fmd?: string
        }
        replaceCalled = true
    ...
    const replaceBtn = page.getByRole('button', {
      name: /Replace fingerprint/i,
    })
    if (await replaceBtn.isVisible()) {
      await replaceBtn.click()
    }

    const fpSection: Locator = page.getByTestId(
    ...
    await expect(page.getByText(/Fingerprint Enrolled/i)).toBeVisible({
      timeout: 10000,
    })
