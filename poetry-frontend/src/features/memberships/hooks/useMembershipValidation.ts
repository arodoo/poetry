...
export function useMembershipValidation() {
  ...
  const checkSellerCode = async (code: string): Promise<ValidationResult> => {
    if (!code)
      return {
        isValid: false,
        errorKey: 'ui.memberships.validation.sellerCode',
      }
    ...
    try {
      ...
      const found = page.content?.find(
        (c) => c.code === code && c.status?.toUpperCase() === 'ACTIVE'
      )
      ...
      return found
        ...
        : {
            isValid: false,
            errorKey: 'ui.memberships.validation.sellerCodeInvalid',
          }
    } finally {
  ...
  const checkUserEligibility = async (
    ...
    try {
      ...
      if (!hasFingerprint) {
        return {
          isValid: false,
          errorKey: 'ui.memberships.validation.noFingerprint',
        }
      }
