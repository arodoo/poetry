...
export function useChartDetailsTable(data: DashboardMetrics | undefined, chartId: string | undefined, t: (key: string) => string) {
    const tableData = useMemo(() => {
        if (!data || !chartId) return [];
        const chartData = data[chartId as keyof DashboardMetrics];
        if (!chartData) return [];
        return Object.entries(chartData as Record<string, unknown>).map(([key, value]) => ({ key, value: Number(value) }));
    }, [data, chartId]);
    ...
    const columns = useMemo(() => {
        ...
        return [
            { key: 'key', header: getHeaderKey(chartId), accessor: (row: { key: string; value: number }) => formatKey(row.key, chartId) },
            { key: 'value', header: getHeaderValue(chartId), accessor: (row: { key: string; value: number }) => row.value },
        ];
    }, [t, chartId]);
