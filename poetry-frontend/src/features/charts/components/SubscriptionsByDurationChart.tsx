 ...
 * Purpose: Bar chart showing number of subscriptions by duration.
...
export function SubscriptionsByDurationChart({ data }: { data: Record<string, number> | undefined }): ReactElement | null {
  const t = useT();
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] p-4 shadow-sm">
      <h3 className="mb-2 text-lg font-semibold text-[var(--color-text)]">
        {t('ui.charts.subscriptionsByDuration')}
      </h3>
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--color-divider)" />
            <XAxis dataKey="duration" tick={{ fill: 'var(--color-text-muted)', fontSize: 12 }} axisLine={false} tickLine={false} />
      ...
      <div className="mt-4 flex justify-end">
        <Button variant="secondary" size="sm" onClick={() => { void navigate(`/${locale ?? 'en'}/charts/details/subscriptionsByDuration`); }}>
