...
export function SellerCodesByStatusChart({ data }: { data: Record<string, number> | undefined }): ReactElement | null {
  const t = useT();
  const { locale } = useParams();
  const navigate = useNavigate();
  if (!data) return null;
  const chartData = formatPieData(data).map((entry, index) => ({ ...entry, fill: CHART_COLORS[(index + 7) % CHART_COLORS.length] ?? '#8884d8' }));
  ...
  return (
    <div className="flex h-[350px] flex-col rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] p-4 shadow-sm">
      ...
      <div className="mt-4 flex justify-end">
        <Button variant="secondary" size="sm" onClick={() => { void navigate(`/${locale ?? 'en'}/charts/details/sellerCodesByStatus`); }}>
