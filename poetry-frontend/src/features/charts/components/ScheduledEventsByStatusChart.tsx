...
export function ScheduledEventsByStatusChart({ data }: { data: Record<string, number> | undefined }) {
  const t = useT();
  if (!data) return null;
          ...
          <PieChart>
            <Pie data={chartData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value">
              {chartData.map((_, index) => (
                <Cell key={`cell-${index}`} fill={CHART_COLORS[(index + 5) % CHART_COLORS.length] as string} />
              ))}
            </Pie>
