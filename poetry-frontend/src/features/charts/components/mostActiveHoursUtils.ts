...
export function computeActiveHoursData(data: Record<string, number>): { name: string; value: number }[] {
    ...
    for (let i = 0; i < 24; i++) {
        ...
        result.push({ name: `${String(h12)}:00 ${ampm}`, value: hoursMap.get(i) ?? 0 });
    }
    return result;
}
...
export function computeActiveHoursKpis(
    ...
    Object.entries(daysData).forEach(([isoDayStr, count]) => {
        if (count > bDayVal) {
            ...
            if (!isNaN(iso)) {
                ...
                bDayName = new Intl.DateTimeFormat(locale ?? 'en', { weekday: 'long' }).format(d);
            }
        }
    });
    ...
    return {
        ...
        busiestDayInfo: bDayVal > 0 ? `${bDayName.charAt(0).toUpperCase() + bDayName.slice(1)} (${String(bDayVal)})` : '--'
    };
}
